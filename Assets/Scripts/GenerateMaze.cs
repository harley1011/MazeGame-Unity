...
public class GenerateMaze : MonoBehaviour {
	...
	public void Start () {
		this.transform.position = new Vector3(sizeOfMap*sizeOfMap, 2 *(sizeOfMap*sizeOfMap), sizeOfMap*sizeOfMap );
	...
	public void makeWalls() // Function is used to instantiate walls from a 2D array
	{
		...
		{
			positionOfWall = new Vector3( wallScaleX * i, 0, 0 ); // Instantiate top border
			...
			for ( int k = 0; k <= sizeOfMap - 1; k++ )
			{
				...
				{	
					positionOfWall = new Vector3( -wallScaleX/2, 0, wallScaleX * k +  wallScaleX/2 + wallScaleZ/2);
					mazeWalls[i,k] = (GameObject)Instantiate(wallPrefab, new Vector3( -wallScaleX/2, 0, wallScaleX * k +  wallScaleX/2 + wallScaleZ/2), Quaternion.AngleAxis(90, Vector3.up));		
				}
				if ( maze[i,k].bottom ) // Checks if the cellbocks bottom wall is true or false
				{  
					positionOfWall = new Vector3( wallScaleX * i + wallScaleX/2, 0, wallScaleX * k  + wallScaleX/2 + wallScaleZ/2);
				...
				{
					positionOfWall = new Vector3( wallScaleX * i , 0, wallScaleX * k + wallScaleX + wallScaleZ/2);
